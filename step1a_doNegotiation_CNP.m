...
for i = 1:length(communicationCandidates(:,1))   
   ...
   if coordination==1 && flightsData(acNr1,25)==2 %If coordiation, all alliance members become manager
       ...
       bidbook(find(bidbook(:,1)==acNr1),:)=0; %if any bids made to me, ditch these       
    ...
    if(flightsData(acNr1,29)==0)
        ...
        if any(any(potentialManagers)) == 1 %Check if there are any potential managers
            ...
            if coordination==1 && flightsData(acNr1,25)==2 %if coordination & alliance
               fuelSavingsOffer = min(0.99,(ratio_managers_contractors/3+flightsData(acNr1,30)/15)-0.2)*potentialFuelSavings; %so if there are more managers, bid lesss
